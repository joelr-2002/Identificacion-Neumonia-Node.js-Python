...
#para que guarde el modelo cada epoca
checkpoint = ModelCheckpoint("elmodelo2.h5", monitor='val_loss', verbose=1, save_best_only=True, save_weights_only=False, mode='auto', period=1)
...
classes = glob('C:/xampp/htdocs/ProyectoIA/python/chest_xray/train/*') #Finding how many classes present in our train dataset.
...
train_datagen = ImageDataGenerator(
	rescale = 1./255, #importing our dataset to keras using ImageDataGenerator in keras.
  shear_range = 0.2,
  zoom_range = 0.2,
  horizontal_flip = True
)
testing_datagen = ImageDataGenerator(rescale =1. / 255)
training_set = train_datagen.flow_from_directory('C:/xampp/htdocs/ProyectoIA/python/chest_xray/train', #inserting the images.
  target_size = (224, 224),
  batch_size = 4,
  class_mode = 'categorical'
)
test_set = testing_datagen.flow_from_directory(
	'C:/xampp/htdocs/ProyectoIA/python/chest_xray/test',
  target_size = (224, 224),
  batch_size = 4,
  class_mode = 'categorical'
)
...
#Mostrando los pesos por cada capa
...
final_model.save('elmodelo2.h5') #Saving the model file.
